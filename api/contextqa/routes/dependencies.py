...
from contextqa.services.context import PineconeManager, LocalManager, BatchProcessor
...
def context_procesor() -> BatchProcessor:
    """Return a BatchProcessor for a specific context manager chosen using the dynamic store settings
    ...
    store_settings = get_or_set(kind="store")
    if store_settings["store"] == "chroma":
        context_manager = LocalManager()
    else:
        context_manager = PineconeManager()

    return BatchProcessor(manager=context_manager)
